...
for i in range(len(df)):
    ...
    if minute in valid_intervals:
        frequency_data.append([df.iloc[i,2], df.iloc[i,5]])
...
#Checking if there is missing any values and finding their position
missing_values = []
for i in range(len(frequency_data)-1):
    b=abs(frequency_data[i+1,0]-frequency_data[i,0])
    if b!=3 and b!=57: 
...
for pos in missing_values:
    if frequency_data[pos,0] == 58:
        value_to_add = [1,50]
    else:
        value_to_add = [frequency_data[pos,0]+3,50]
...
frequency_df = pd.DataFrame(frequency_data, columns=['Column 2', 'Column 5'])
