...
price_winter = FCR_N[:, np.concatenate((np.arange(334, 365), np.arange(0, 59)))]
price_spring = FCR_N[:, 59:151]
price_summer = FCR_N[:, 151:243]
price_autumn = FCR_N[:, 243:334]

price_winter_mean=np.zeros((24))
for i in range(24):
    price_winter_mean[i]=sum(price_winter[i])/len(price_winter[i])

price_spring_mean=np.zeros((24))
for i in range(24):
    price_spring_mean[i]=sum(price_spring[i])/len(price_spring[i])

price_summer_mean=np.zeros((24))
for i in range(24):
    price_summer_mean[i]=sum(price_summer[i])/len(price_summer[i])

price_autumn_mean=np.zeros((24))
for i in range(24):
    price_autumn_mean[i]=sum(price_autumn[i])/len(price_autumn[i])
...
axs[0, 0].plot(price_winter_mean)
...
axs[0, 1].plot(price_spring_mean)
...
axs[1, 0].plot(price_summer_mean)
...
axs[1, 1].plot(price_autumn_mean)
...
# Plot mean FCR-N price during winter
axs[0, 0].plot(price_winter_mean_FCR_D_up)
axs[0, 0].set_title('Winter')
axs[0, 0].set_ylabel('SEK/MWh')
axs[0, 0].set_xlabel('Hour of the day')
axs[0, 0].set_xlim(0, 23)
...
# Plot mean FCR-N price during spring
axs[0, 1].plot(price_spring_mean_FCR_D_up)
axs[0, 1].set_title('Spring')
axs[0, 1].set_ylabel('SEK/MWh')
axs[0, 1].set_xlabel('Hour of the day')
axs[0, 1].grid(True)
...
# Plot mean FCR-N price during summer
axs[1, 0].plot(price_summer_mean_FCR_D_up)
axs[1, 0].set_title('Summer')
axs[1, 0].set_ylabel('SEK/MWh')
axs[1, 0].set_xlabel('Hour of the day')
axs[1, 0].set_xlim(0, 23)
...
# Plot mean FCR-N price during autumn
axs[1, 1].plot(price_autumn_mean_FCR_D_up)
axs[1, 1].set_title('Autumn')
axs[1, 1].set_ylabel('SEK/MWh')
axs[1, 1].set_xlabel('Hour of the day')
axs[1, 1].grid(True)
