...
# yearly_load = load_data.reshape(-1, order='F')
...
# Plotting the daily load
plt.figure(figsize=(10, 5))
plt.plot(range(len(load_data_hourly)), load_data_hourly, alpha=0.5)
#plt.plot(range(len(yearly_spot_price)), yearly_spot_price, label='Spot Price')
#plt.fill_between(range(len(load_data_hourly)), load_data_hourly, alpha=0.3)
plt.xlabel('Hours')
plt.ylabel('kWh')
#plt.title('Electricity Load')
#plt.legend()
plt.xlim(0, 8760)  # Set x-axis limits to start at 0 and end at 364
plt.ylim(0, (max(load_data_hourly)+1)) 
#plt.grid(True)
plt.show(block=False)
plt.show()
...
#FCR-D revenue   
FCR_D_up_price_data=(frequency_price.FCR_D_up_2024_8760)/1000
...
#FCR-D down revenue
FCR_D_down_price_data=(frequency_price.FCR_D_down_2024_8760)/1000
